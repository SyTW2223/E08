...
describe('POST / - creation of a new account', () => {
    test('It must require a username', async () => {
        await api
            ...
            expect(response.body.message).toBe("Account validation failed: username: A username is required");
        });
    });
    test('It must require an account name', async () => {
        await api
            ...
            .expect(response => {
            expect(response.status).toBe(400);
            expect(response.body.message).toBe("Account validation failed: accountName: An account name is required");
        });
    });
    test('It must require a email', async () => {
        await api
            ...
            .expect(response => {
            expect(response.status).toBe(400);
            expect(response.body.message).toBe("Account validation failed: email: An email is required");
        });
    });
    test('It must require a password', async () => {
        await api
            ...
            .expect(response => {
            expect(response.status).toBe(400);
            expect(response.body.message).toBe("Account validation failed: password: A password is required");
        });
    });
